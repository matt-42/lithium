...
//
// Single header version the li_http_client library.
...
namespace s {                                                           \
struct NAME##_t : li_http_client::symbol<NAME##_t> {                         \
                                                                        \
...
namespace li_http_client {
  ...
  namespace impl
  {
    ...
    template <typename S>
    auto make_json_object_member(const li_http_client::symbol<S>&);
    ...
    auto make_json_object_member(const function_call_exp<S, A...>& e)
    {
      ...
      ::li_http_client::tuple_map(e.args, parse);
      return res;
    }
...
struct http_client {
  ...
  template <typename... A> inline auto operator()(int http_method, const std::string_view& url, const A&... args) {
    ...
    // Get params
    auto get_params = li_http_client::get_or(arguments, s::get_parameters, mmm());
    bool first = true;
    li_http_client::map(get_params, [&](auto k, auto v) {
      if (first)
        url_ss << '?';
      else
        url_ss << "&";
      ...
      url_ss << li_http_client::symbol_string(k) << '=' << escaped;
      first = false;
      curl_free(escaped);
    });
    ...
    if (is_urlencoded) { // urlencoded
      ...
      auto post_params = li_http_client::get_or(arguments, s::post_parameters, mmm());
      first = true;
      li_http_client::map(post_params, [&](auto k, auto v) {
        if (!first)
          post_stream << "&";
        post_stream << li_http_client::symbol_string(k) << "=";
      ...
      rq_body = li_http_client::json_encode(li_http_client::get_or(arguments, s::post_parameters, mmm()));
