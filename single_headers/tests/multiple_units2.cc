...
auto db2 = sqlite_database("blog_database.sqlite");

// We declare the users ORM.
//   Per user, we need a unique id, login and a password.
auto users2 = sql_orm_schema(db2, "blog_users")
...
auto sessions2 = hashmap_http_session("test_cookie", s::user_id = -1);
...
auto auth2 = http_authentication(sessions2, users2, s::login, s::password);
...
//
auto posts2 = sql_orm_schema(db2, "blog_posts")
                  .fields(s::id(s::auto_increment, s::primary_key) = int(),
                  ...
                  .callbacks(
                      ...
                      s::before_insert =
                          [](auto& post, http_request& req, http_response& resp) {
                            auto u = auth2.current_user(req, resp);
                            if (!u)
...
http_api api2() {
  ...
  my_api.add_subapi("/auth", http_authentication_api(auth2));
  // Generate /post/[create|update|remove]
  my_api.add_subapi("/post", sql_crud_api(posts2));
