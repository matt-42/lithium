...
namespace iod
{
  ...
  namespace impl
  {
    ...
    inline MYSQL* open_mysql_connection(OPTS&&... opts)
    {
      ...
      static_assert(has_key(options, _password), 'open_mysql_connection requires the _password argument');
      
      MYSQL* con;
      con = mysql_init(con);
      con = mysql_real_connect(con,
                               get_c_str(options.host),
                               get_c_str(options.user),
                               get_c_str(options.password),
                               get_c_str(options.database),
                               0,NULL, 0);
  ...
  struct mysql_connection
  {
    ...
    template <typename OPTS>
    inline mysql_connection(OPTS&&... opts)
      : con_(con)
    {
