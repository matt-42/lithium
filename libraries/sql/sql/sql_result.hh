...
template <typename I> struct sql_result {
   ...
   * With postgresql, it requires the previous command to use the "INSERT [...] returning id;" syntax.
   *
   ...
   *    read(li::metamap<...>& )
   *        fill the metamap according to the current row. The metamap (value types and keys) must match
   *        the fields (types and names) of the request.
   *    read(A& a, B& b, C& c, ...)
   *        fill a, b, c,... with each field of the current row. Types of a, b, c... must
   *        match the types of the fields.
   *        supported types : std::string, integer and floating numbers.
   * @return T the result value.
   */
  template <typename... T> void read(T&... fields) { impl_->read(std::forward<T>(fields)...); }
